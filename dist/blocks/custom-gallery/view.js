...
class MosneCarousel {
  ...
  init() {
    ...
    Array.prototype.forEach.call(this.elements, function (el, i) {
      ...
      button.innerHTML = '<span class="sr-only">Go to slide ' + (i + 1) + '<span>';
    ...
    this.observer = new IntersectionObserver(function (entries) {
      entries.forEach(function (entry) {
        ...
        if (entry.isIntersecting) {
          ...
          self.counterActive = self.counterActive.filter(e => e !== idx + 1);
        }
      });
  ...
  resetClass(containerEl, classToRemove) {
    const targetElements = containerEl.querySelectorAll('.' + classToRemove);
